...
from src.pages import handle_pages
from src.pdf_reader import get_fitz_doc_obj
from src.file_utils import coerce_file_to_pdf
from src.ocr import get_ocr_filename
...
def single_process(
    data_inputs: typing.Tuple[typing.Callable, str, str, bool, int, bool, str]
) -> None:
    """
    Args:
    ...
    (f_name, meta_data, ocr_missing_doc, num_ocr_threads, force_ocr, out_dir) = (
        data_inputs
    )
    ...
    try:
        if not meta_data:
            ...
            loc_meta_path = Path(
                Path(meta_data) if Path(meta_data).is_dir() else Path(meta_data).parent,
